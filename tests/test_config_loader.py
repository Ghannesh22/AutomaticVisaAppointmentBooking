...
class EnvLoadingTests(unittest.TestCase):
    ...
    def test_load_project_env_overrides_inherited_empty_values(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            env_path = Path(temp_dir) / ".env"
            env_path.write_text("CUSTOM_FIRST_NAME=Second\n", encoding="utf-8")

            with patch.dict(os.environ, {"CUSTOM_FIRST_NAME": ""}, clear=True):
                load_project_env(env_path)

                self.assertEqual(os.environ["CUSTOM_FIRST_NAME"], "Second")
...
""",
            ...
            self.assertEqual(config.profile_for_month("2026-07").name, "applicant_a")
            self.assertEqual(config.profile_for_month("2026-10").name, "applicant_b")
            self.assertEqual(
                config.target_months_for_open_profiles({"applicant_a"}),
                ("2026-09", "2026-10"),
            )
